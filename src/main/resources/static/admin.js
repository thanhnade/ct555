...
- name: Khởi tạo master node
  ...
  tasks:
    ...
    - name: Tạo thư mục .kube cho user
      file:
    ...
    - name: Copy admin config cho user
      copy:
        src: /etc/kubernetes/admin.conf
...
- name: Chuẩn bị môi trường K8s (tắt swap, bật br_netfilter, cấu hình sysctl)
  hosts: all
  become: yes
  tasks:
    - name: Tắt swap
    ...
    - name: Comment swap trong /etc/fstab
      lineinfile:
        path: /etc/fstab
        regexp: '^([^#].*\\sswap\\s+sw\\s+.*)$'
        line: '# \\1'
...
- name: Reset cluster K8s
  ...
  tasks:
    ...
    - name: Hiển thị thông báo reset hoàn thành
      debug:
        msg: "Cluster K8s đã được reset hoàn toàn"`,
  ...
  if (currentClusterId && playbookContent.includes('{{ cluster_id | default(1) }}')) {
    finalPlaybookContent = playbookContent.replace('{{ cluster_id | default(1) }}', currentClusterId);
  }
